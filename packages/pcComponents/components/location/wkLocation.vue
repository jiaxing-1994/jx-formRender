<template>
  <div class="w_100 flex flex-top">
    <p
      class="ellipsis flex1 cs-p"
...
export default defineComponent({
  ...
  props: {
    value: {
      type: [Object, null] as PropType<LocationType | null>,
      default: null,
    },
    isEdit: {
  ...
  setup(props, context) {
    const modelValue = ref<LocationType>({
      longitude: props.value?.longitude || 0,
      latitude: props.value?.latitude || 0,
      address: props.value?.address || "",
    });
    const modalLocationInfo = ref<LocationType>();
    watch(
      () => props.value,
      () => {
        if (props.value) {
    ...
    const initMapInstance = (mapDom: HTMLElement) => {
      if (!mapInstance.value) {
        ...
        centerMarker.value = mapInstance.value.addMarker([
          modalLocationInfo.value?.longitude,
          modalLocationInfo.value?.latitude,
        ]);
      }
    };
    // 初始化定位信息
    const initLocation = async (): Promise<LocationType> => {
    ...
    initLocation().then((data) => {
      ...
      context.emit("update:value", { ...modelValue.value });
    });
